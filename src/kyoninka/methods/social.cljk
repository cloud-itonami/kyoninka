(ns kyoninka.methods.social
  "social.cljc — 許認可 — Robotaxi Legal-Deployment Permitting DRY-RUN self-publication seed (ADR-2606272355, auto-seeded R0).

  This is a GENERIC self-publication projection (draft-observation-post below builds an
  app.bsky.feed.post-shaped record), mechanically derived from kyoninka's own
...
(def DISCLAIMER
  (str "【観測ミラー / accountability map — NOT a verdict, NOT advice, 非断定】 "
       "許認可 — Robotaxi Legal-Deployment Permitting が既知の観測から編んだ事実の要約です。"))
...
(defn draft-observation-post
  "A dry-run observation post over kyoninka's own domain (Robotaxi legal-deployment permitting actor — answers, per jurisdiction and with an immutable paper trail, \"are we legally clear to operate…)."
...
(defn build-live
  ...
  [& _args]
  (throw (ex-info (str "kyoninka R0: live social posting is Council Lv6+ + operator + "
